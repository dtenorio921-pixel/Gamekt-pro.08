...
public class Container {
    ...
    // Optimized for 4GB RAM ARM64 devices (e.g. Moto G35) — balanced for both light and heavy games:
    // - MESA_SHADER_CACHE_MAX_SIZE=256MB: saves RAM vs previous 512MB default
    // - WINEDEBUG=-all: disables Wine debug output — frees CPU/RAM for the game
    // - STAGING_SHARED_MEMORY=1: better shared-memory handling on low-RAM devices
    // - DXVK_ASYNC=1: enables async shader compilation — eliminates stuttering while shaders compile
    // - ZINK_DESCRIPTORS=lazy + ZINK_DEBUG=compact: reduced GPU descriptor overhead
    // - mesa_glthread=true: offloads GL calls to a separate thread — better GPU utilization
    // - WINEESYNC=1: eventfd sync — far lower CPU overhead than standard wineserver sync
    // - MESA_VK_WSI_PRESENT_MODE=mailbox: low-latency frame presentation
    public static final String DEFAULT_ENV_VARS = "WRAPPER_MAX_IMAGE_COUNT=0 ZINK_DESCRIPTORS=lazy ZINK_DEBUG=compact MESA_SHADER_CACHE_DISABLE=false MESA_SHADER_CACHE_MAX_SIZE=256MB mesa_glthread=true WINEESYNC=1 MESA_VK_WSI_PRESENT_MODE=mailbox TU_DEBUG=noconform DXVK_FRAME_RATE=60 PULSE_LATENCY_MSEC=144 WINEDEBUG=-all STAGING_SHARED_MEMORY=1 DXVK_ASYNC=1";
    // 1280x720 is optimal for Moto G35 (native 1600x720) — good balance of quality and performance
    ...
    // videoMemorySize reduced from 2048 to 1024 MB — appropriate for devices with 4GB RAM (e.g. Moto G35)
    // maxDeviceMemory=1024 limits DXVK's device memory consumption to avoid OOM on low-RAM devices
    public static final String DEFAULT_DXWRAPPERCONFIG = "version=" + DefaultVersion.DXVK + ",framerate=0,maxDeviceMemory=1024,async=" + DefaultVersion.ASYNC + ",asyncCache=" + DefaultVersion.ASYNC_CACHE + ",vkd3dVersion=" + DefaultVersion.VKD3D + ",vkd3dLevel=12_1" + ",ddrawrapper=" + Container.DEFAULT_DDRAWRAPPER + ",csmt=3" + ",gpuName=NVIDIA GeForce GTX 480" + ",videoMemorySize=1024" + ",strict_shader_math=1" + ",OffscreenRenderingMode=fbo" + ",renderer=gl";
